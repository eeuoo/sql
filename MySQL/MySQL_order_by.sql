...
select count(*) from Student; --  count(*) 하고 group by 하지 않으면 그냥 레코드 수 return

select * from Student order by birth limit 10 ;

select * from Student where name like '김%' limit 10 ;

select * from Student where addr = '서울' order by id desc;

select * from Student where addr = '서울' and  birth between '900101' and '999999' order by birth asc;

select s.name , p.name from Subject s inner join Prof p where s.prof = p.id ;
select s.name , p.name from Subject s inner join Prof p on s.prof = p.id ;

-- sub query를 이용해 join 한 것처럼 보여주는 식
select sbj.name , (select name from Prof where id = sbj.prof) as 'subject_prof_name' from Subject sbj; 
